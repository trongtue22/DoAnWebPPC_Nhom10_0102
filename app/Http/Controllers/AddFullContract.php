...
class AddFullContract extends Controller
{
    ...
    # Hàm này giúp check các data mà người dùng nhập => Data lấy từ bên route với phương thức POST 
    public function checkFormatContract(Request $request)
    {
        $request->validate([
            ...
            'SSN'           => 'required',
            // Fix 
            'phoneNumber'   => 'nullable|regex:/^[0-9]+$/|digits_between:6,11', 
            // Sửa 
            'YearOfBirth'   => 'nullable|gte:0|before_or_equal:'.date('Y'),
            'DateContract'  => 'nullable|date|after:1990-01-01|before_or_equal:'.date('Y-m-d'),
            'Price'         => 'nullable|gte:0',
            'Deposit'       => 'nullable|gte:0',
            'Remain'        => 'nullable|gte:0',
            'address'       => 'nullable|min:3|max:60', 
                ...
                'phoneNumber.regex'                 => 'Số điện thoại chỉ được chứa các chữ số và phải lớn hơn 0',
                'phoneNumber.digits_between'        => 'Số điện thoại phải có hơn 6 và nhỏ hơn 11 ký tự',
                ...
                'DateContract.before_or_equal'      => 'Vui lòng chọn ngày lập hợp đồng từ 1990 đến hiện tại.',
                'Deposit.gte'                       => 'Không được nhập số âm',
                'Remain.gte'                        => 'Không được nhập số âm',
                'Price.gte'                         => 'Không được nhập số âm', 
                // Sửa
                'YearOfBirth.gte'                   => 'Không được nhập số âm', 
                'phoneNumber.gte'                   => 'Không được nhập số âm', 
                // 
                'fullName.min'                      => 'Tên chỉ được nằm trong khoảng 3 tới 50 ký tự',
                'fullName.max'                      => 'Tên chỉ được nằm trong khoảng 3 tới 50 ký tự',
                'address.min'                       => 'Địa chỉ phải nhiều hơn 3 ký tự',
                'address.max'                       => 'Địa chỉ phải ít hơn 60 ký tự',    
        ]);        
