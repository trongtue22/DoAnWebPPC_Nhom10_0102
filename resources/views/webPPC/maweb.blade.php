...
<filed class="form-popup" id="myForm">
  ...
  <form action="/web" method="POST" class="form-container">
    ...
    @enderror
     ...
     <div class="mb-3">
      <label for="phoneNumber"><b> Số điện thoại </b></label>
       <input type="text" placeholder="Nhập số điện thoại" name="phoneNumber" class="form-control" 
       @if(!$errors->has('phoneNumber')) value="{{ old('phoneNumber') }}" @endif>
    ...
    <div class="mb-3">
      ...
      <input type="date" placeholder="Nhập ngày lập hợp đồng" name="DateContract" class="form-control" 
           @if(!$errors->has('DateContract')) value="{{ old('DateContract') }}" @endif>
    ...
    <label for="Price"><b>Giá</b></label>
      <input type="number" step="1" placeholder="Nhập giá" name="Price" class="form-control" 
      @if(!$errors->has('Price')) value="{{ old('Price') }}" @endif>
    ...
    <label for="Deposit"><b>Tiền cọc</b></label>
      <input type="number" step="1" placeholder="Nhập tiền cọc" name="Deposit" class="form-control" 
      @if(!$errors->has('Deposit')) value="{{ old('Deposit') }}" @endif>
    ...
    <label for="Remain"><b>Tiền còn lại</b></label>
      <input type="number" step="1" placeholder="Nhập tiền còn lại" name="Remain" class="form-control" 
      @if(!$errors->has('Remain')) value="{{ old('Remain') }}" @endif>
      @error("Remain")
        <span style='color:red;'>{{$message}}</span>   
      @enderror
    </div>
  ...
  <select name="PropertyID">
    @foreach($MangChuaProperty as $item)
        <option value="{{ $item->ID }}" @if(old('PropertyID') == $item->ID) selected @endif>
            {{ $item->Property_Name }}
        </option>
    @endforeach
  </select>
